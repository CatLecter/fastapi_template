...
class TicketService:
    def __init__(self, transaction: Transaction, user_repository: UserRepository, ticket_repository: TicketRepository):
    ...
    async def get_by_user_mobile_phone(self, mobile_phone: str) -> list[ResponseBriefTicket]:
        async with self.transaction:
            ...
            if not is_user_exist:
                raise HTTPException(
                    status_code=HTTPStatus.NOT_FOUND, detail=f'User with mobile phone {mobile_phone} not found'
                )
            ...
            if not tickets:
                raise HTTPException(
                    status_code=HTTPStatus.NOT_FOUND, detail=f'Tickets with mobile phone {mobile_phone} not found'
                )
            return tickets
