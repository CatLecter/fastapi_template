...
from app.schemes.external import RequestTicket, ResponseBriefTicket
from app.schemes.external import ResponseTicket
from app.services import TicketService

router = APIRouter(prefix='/ticket', tags=['Tickets'])


@router.post(path='/', response_model=ResponseTicket)
async def add_ticket(
    item: RequestTicket, ticket_service: TicketService = Depends(get_ticket_service)
) -> ResponseTicket:
...
async def get_ticket_by_id(
    ticket_id: UUID, ticket_service: TicketService = Depends(get_ticket_service)
) -> ResponseTicket:
...
async def get_tickets_by_user_mobile_phone(
    mobile_phone: str, ticket_service: TicketService = Depends(get_ticket_service)
